...
public:
	...
	CMFCPropertyGridProperty* AddGrouppedProperty(LPCTSTR pszGroup, LPCTSTR pszName, const COleVariant& value, LPCTSTR pszDescription = NULL, BOOL bAllowEdit = TRUE);
...
public:
	T m_min{0}, m_max{0};
	CString m_strFormat;
	typedef misc_internal::TNumberVariableFormat<t_bInteger, t_bSigned, t_cSize> CNumberFormat;
...
public:
	...
	virtual void SetValueT(const T& v) {
		__super::SetValue((LPCTSTR)Format(m_strFormat, v));
	}
...
using CPropItemInt = TPropItemSimple<intXX_t>;
using CPropItemUInt = TPropItemSimple<uintXX_t>;
...
using CPropItemFormattedInt		= TPropItemFormatted<intXX_t>;
using CPropItemFormattedUInt	= TPropItemFormatted<uintXX_t>;
...
public:
	qtl::TUniquePtrVector<IPropItemBase> m_subItems;
	...
	virtual bool Sync(bool bStoreToVar) {
		...
		#else
			auto pos = std::find_if(m_subItems.begin(), m_subItems.end(), [pSubItem](const IPropItemBase& A) ->bool { return A.m_pProperty == pSubItem; });
		#endif
	...
	IPropItemBase* AddSubItem(IPropItemBase* pSubItem) {
		...
		return m_subItems.Push(pSubItem) ? pSubItem : nullptr;
	}
...
protected:
	void AddOption(CMFCPropertyGridProperty* pProperty, LPCTSTR pszOptions) {
		...
		SplitString(pszOptions, _T(';'), strs);
		for (const auto& str : strs)
			pProperty->AddOption(str, false);
	}
};

//-----------------------------------------------------------------------------
class CPropGroupSet : public qtl::TUniquePtrVector<CPropItemGroup> {
protected:
...
public:
	...
	bool Sync(bool bStoreToVar) {
		...
		#else
			auto pos = std::find_if(begin(), end(), [pProp](const CPropItemGroup& self) -> bool { return (self.m_pProperty && pProp) ? (self.m_pProperty == pProp) : false; } );
		#endif
