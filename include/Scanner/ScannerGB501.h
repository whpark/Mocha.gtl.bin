...
class AFX_EXT_CLASS_SCANNER CScannerGB501 : public IScanner {
public:
	CScannerGB501(int iBoard = 1) : IScanner(iBoard) { 	LoadDriver(nullptr); }
	virtual ~CScannerGB501() {}

	virtual eSCANNER_TYPE GetScannerType() { return SCANNER_GB501; }

	static bool LoadDriver(LPCTSTR pszPath);
	static bool ReleaseDriver();

	//virtual UINT init_rtc5_dll(void);
	//virtual void free_rtc5_dll(void);
	...
	virtual UINT select_board(int iBoard);
	virtual UINT get_dll_version(void);
	//virtual UINT transform(long& Sig1, long& Sig2, const PULONG_PTR Ptr, UINT Code);
	...
	//virtual long load_stretch_table(const char* Name, long No);
	...
	virtual void set_control_mode(unsigned long control_mode, unsigned long count_sel, unsigned long count_mode);
	...
	//virtual void get_waveform(UINT Channel, UINT Number, PULONG_PTR Ptr);
	...
	//virtual void get_transform(UINT Number, PULONG_PTR Ptr1, PULONG_PTR Ptr2, PULONG_PTR Ptr, UINT Code);
	...
	//virtual void set_mcbsp_out_ptr(UINT Number, const PULONG_PTR SignalPtr);
	...
	//virtual UINT upload_transform(const PULONG_PTR Ptr);
	...
	//virtual void get_values(const PULONG_PTR SignalPtr, PULONG_PTR ResultPtr);
	...
	//virtual void set_piso_control(UINT l1, UINT l2) {}	// is not needed for RTC5
	...
	// Board-Index-dependent
	//
	//virtual UINT select_rtc() { return 0; };
